...
public:
    ...
    bool onLoad()
    {
        ...
        if (!m_shader.loadFromFile("resources/pixelate.frag", sf::Shader::Fragment))
            return false;
        m_shader.setUniformSampler2D("texture", sf::Shader::CurrentTexture);
    ...
    {
        m_shader.setUniformFloat("pixel_threshold", (x + y) / 30);
    }
...
public:
    ...
    {
        m_shader.setUniformFloat("wave_phase", time);
        m_shader.setUniformVec2("wave_amplitude", sf::Vector2f(x * 40, y * 40));
        m_shader.setUniformFloat("blur_radius", (x + y) * 0.008f);
    }
...
public:
    ...
    void onUpdate(float time, float x, float y)
    {
        ...
        m_shader.setUniformVec2("storm_position", sf::Vector2f(x * 800, y * 600));
        m_shader.setUniformFloat("storm_inner_radius", radius / 3);
        m_shader.setUniformFloat("storm_total_radius", radius);
        m_shader.setUniformFloat("blink_alpha", 0.5f + std::cos(time * 3) * 0.25f);
    }
...
public:
    ...
    void onUpdate(float time, float x, float y)
    {
        m_shader.setUniformFloat("edge_threshold", 1 - (x + y) / 2);
